...
exports.coursesPage = async (req, res) => {
    try {
        ...
        if (req.session.username == undefined) {
            ...
            res.render("coursesPage.ejs", { userConnected: 0, courses: rows, cart: cart });
        } else {
            // Render the courses page without the login button if connected
            res.render("coursesPage.ejs", { userConnected: 1, courses: rows, cart: cart });
        }
    } catch (error) {
        console.log(error);
    }
...
exports.login = (req, res) => {
    try {
        ...
        if (req.query.source != undefined) {
            let message = "You are registered for :";
            cart.getCourses().forEach(async (course) => {
                // Format the 'res.send()' message and saves registration to database
                message += "<br/>-- " + course.name;
                await connection.query("INSERT INTO registration SET ?", {
                    username: req.session.username,
                    id: course.id,
...
exports.addToCart = async (req, res) => {
    try {
        ...
        cart.getCourses().sort((a, b) => a.id - b.id); // Sort by id for cartPage display
